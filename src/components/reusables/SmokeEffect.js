  ...
  useEffect(() => {
    ...
    function render() {
      ...
      for (let i = parts.length - 1; i >= 0; i--) {
        if (parts[i].y < 0 || parts[i].lifeTime > maxLifeTime) {
          ...
          ctx.translate(parts[i].x, parts[i].y);
          ctx.rotate(parts[i].angle / 180 * Math.PI);
          ctx.drawImage(smokeImage, -parts[i].size / 2, -parts[i].size / 2, parts[i].size, parts[i].size);
          ctx.restore();
        }
      }
      spawn();
    ...
    class Smoke {
      constructor(x, y) {
        ...
        this.velX = Math.floor(Math.random() * 10) - 5;
      }

      update() {
        this.lifeTime = new Date().getTime() - this.startLife;
        this.alpha = 1 - this.lifeTime / maxLifeTime;
        this.size = this.startSize + (this.endSize - this.startSize) * (this.lifeTime / maxLifeTime);
        this.y += this.velY;
        this.x += this.velX;
      }
    }
